...
const lineas = listaBancos.split('\n');


export const arrayBancos : listaBancos[] = lineas.map(linea => {
  // Dividir cada línea en código y nombre
  const [codigo, ...nombreArray] = linea.split(' ');
  const nombre = nombreArray.join(' '); // Reconstruir el nombre del banco si contiene espacios
